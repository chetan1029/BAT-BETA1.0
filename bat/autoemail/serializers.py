...
class EmailCampaignSerializer(serializers.ModelSerializer):
    ...
    def get_email_in_queue(self, obj):
        return EmailQueue.objects.filter(
            ...
            status__name__in=[ORDER_EMAIL_STATUS_SCHEDULED, ORDER_EMAIL_STATUS_QUEUED]
        ).count()
    ...
    def get_email_sent_today(self, obj):
        ...
        return EmailQueue.objects.filter(emailcampaign_id=obj.id, status__name=ORDER_EMAIL_STATUS_SEND, send_date__year=today.year, send_date__month=today.month, send_date__day=today.day).count()

    def get_email_queue_today(self, obj):
        today = datetime.date.today()
        return EmailQueue.objects.filter(emailcampaign_id=obj.id, status__name=ORDER_EMAIL_STATUS_SEND, create_date__year=today.year, create_date__month=today.month, create_date__day=today.day).count()
