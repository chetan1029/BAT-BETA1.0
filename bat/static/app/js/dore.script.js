  ...
  function init() {
    ...
    if (typeof ClassicEditor !== "undefined") {
      ...
      for (ii = 0; ii < x.length; ii++) {
        ...
        var changeCallback = x[ii].getAttribute('data-onChange');
        var initCallback = x[ii].getAttribute('data-onInit');
        var params = x[ii].getAttribute('data-params');
        ...
        ClassicEditor.create(x[ii],{
          ...
          if (changeCallback) {
            editor.model.document.on('change:data', (e) => {
              if (params)
                window[changeCallback](params, editor.getData());
              else
                window[changeCallback](editor.getData());
            });
          }
          if (initCallback) {
            window.onload = function(e){
              if (params)
                window[initCallback](params, editor.getData());
              else
                window[initCallback](editor.getData());
            };
          }
        })
        .catch(
