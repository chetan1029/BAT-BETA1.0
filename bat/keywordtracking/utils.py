...
def clean_global_keyword(csv_file):
    ...
    order = ['Department', 'Search Term', 'Search Frequency Rank', '#1 Clicked ASIN', '#2 Clicked ASIN', '#3 Clicked ASIN']
    ...
    with open(tmp_path, 'w', newline='') as csvfile_out:
        ...
        for item in reader:
            ...
            if not r["Search Term"] or r["Search Term"] == "" or len(r["Search Term"]) <= 0 :
                pass
            else:
